...
function xlsx(file) { 
	...
	var defaultFontName = 'Tahoma';
	var defaultFontSize = 8;

	var result, zip = new JSZip(), zipTime, processTime, s, f, i, j, k, l, t, w, cc, sharedStrings, styles, index, data, val, style, borders, border, borderIndex, fonts, font, fontIndex,
		docProps, xl, xlWorksheets, worksheet, contentTypes = [[], []], props = [], xlRels = [], worksheets = [], id, columns, cols, cell, row, merges, merged, ignored, rendered, fills, fill, fillId,
   ...
   if (typeof file === 'string') { 
   	// Load
		...
		while (w--) { 
			...
			while (++i < l) {
                if(!data[i]) data[i] = [];
                if(!data[i].length) data[i].push({ borders: cell.borders, fake: true });
				j = 0; k = data[i].length - 1;
				...
				while (j <= k) {
					...
					if (val && typeof val === 'string' && !cell.number) {
                        val = escapeXML(val);
						sharedStrings[1]++;
                    ...
                    var key = numAlpha(j) + (i + 1),
                        st = j;

                    while(rendered[key] || merged[key]){
                        if(!ignored[key] && !rendered[key]) s += '<c r="' + key + '"' + (style ? ' s="' + style + '"' : '') + (t ? ' t="' + t + '"' : '') + '/>';
			...
			for (i = 0; i < columns.length; i++) {
                cols.push('<col min="', i+1, '" max="', i+1, '" width="', ((columns[i].width + 16) / 7), '" customWidth="1"/>');
			}
			...
			s = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" mc:Ignorable="x14ac" xmlns:x14ac="http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac">'
				...
				+ ' workbookViewId="0"/></sheetViews><sheetFormatPr defaultRowHeight="18" x14ac:dyDescent="0.25"/>'
				+ cols
				+ '<sheetData>'
				+ s 
				+ '</sheetData>';
		...
		while (--i) { 
            ...
            fillId = 0;
            if(style.bgColor){
                fill = '<fill><patternFill patternType="solid"><fgColor rgb="FF'+style.bgColor+'"/></patternFill></fill>';
                fillId = fills.indexOf(fill);

                if (fillId < 0) {
                    fillId = fills.push(fill) + 1;
                } else {
                    fillId+=2;
                }
            }
		...
		xl.file('workbook.xml', '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">'
			...
			+ (file.activeWorksheet ? 'activeTab="' + file.activeWorksheet + '" ' : '') + 'xWindow="480" yWindow="60" windowWidth="28195" windowHeight="8505"/></bookViews><sheets>'
